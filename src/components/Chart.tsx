...
export function SimpleChart({ data, height = 160, color = '#ff3b30', showPoints = true, showArea = true }: SimpleChartProps) {
  if (!data.length) {
    return (
      <div className="flex items-center justify-center" style={{ height }}>
        <p className="text-xs text-text-gray font-mono">暂无数据</p>
      </div>
    )
  }
        ...
        <g key={i}>
          {p.value != null && (
            <>
              ...
              {data.length <= 14 && (
                <text x={p.x} y={height - 2} textAnchor="middle" fontSize="3" fill="#555555" fontFamily="monospace">
                  {p.label}
                </text>
              )}
            </>
