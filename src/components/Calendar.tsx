...
export function CalendarHeatmap({ data, type = 'workout' }: CalendarHeatmapProps) {
  // Build a 90-day heatmap
  const today = new Date()
  const days: { date: string; label: string; dayNum: number; count: number; intensity: number }[] = []
  ...
  const getColor = (intensity: number) => {
    if (type === 'workout') {
      const colors = ['#1a1d27', '#1e3a5f', '#2563eb', '#6366f1']
      return colors[intensity] || colors[0]
    }
    const colors = ['#1a1d27', '#166534', '#10b981']
    return colors[intensity] || colors[0]
  }
    ...
    <div>
      <div className="flex gap-[2px] overflow-x-auto pb-2">
        {days.map((d, i) => (
          ...
          >
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-bg-card border border-border rounded text-xs text-text-primary whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
              {d.label} · {d.count}次
            </div>
          </div>
        ))}
